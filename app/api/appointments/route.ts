...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    .from('appointments')
    .select('*, patient:patients(full_name, patient_id, id_number), assignee:profiles!assigned_to(full_name, role), requester:profiles!requested_by(full_name, role)')
  ...
  if (idNumbers.length > 0) {
    ...
    if (studentData) {
      ...
      data.forEach((appt: any) => {
        if (appt.patient?.id_number) {
          const profile = studentMap[appt.patient.id_number]
          if (profile) {
...
export async function POST(req: NextRequest) {
  ...
  // For students: find their patient record via id_number
  ...
  if (profile?.role === 'STUDENT') {
    const { data: pat } = await supabase
      ...
      .single()
    if (!pat) return NextResponse.json({ error: 'No patient record found for your ID number. Please visit the clinic first.' }, { status: 404 })
    patientId = pat.id
  }
